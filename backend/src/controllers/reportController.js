...
const csv=(res,name,rows)=>{const esc=(v)=>{let value=String(v??'').replace(/[\r\n\t]+/g,' ');if(/^[=+\-@]/.test(value))value=`'${value}`;return `"${value.replaceAll('"','""')}"`;};const keys=rows.length?Object.keys(rows[0]):['message'];res.setHeader('Content-Type','text/csv; charset=utf-8');res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Content-Disposition',`attachment; filename="${name}.csv"`);res.send([keys.join(','),...rows.map(r=>keys.map(k=>esc(r[k])).join(','))].join('\n'));};
